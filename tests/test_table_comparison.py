...
def test_compare_tables_identical() -> None:
    """Classify fully matching tables as identical."""
    left: dict[str, list[Any]] = {"Category": ["A", "B"], "Total | n": [1, 2]}
    right: dict[str, list[Any]] = {"Category": ["A", "B"], "Total | n": [1, 2]}
...
def test_compare_tables_detects_structure_difference_for_value_mismatch() -> None:
    """Classify value mismatches as structure differences."""
    left: dict[str, list[Any]] = {"Category": ["A", "B"], "Total | n": [1, 2]}
    right: dict[str, list[Any]] = {"Category": ["A", "B"], "Total | n": [1, 3]}
...
def test_compare_tables_detects_structure_difference_for_column_order() -> None:
    """Classify differing column order as a structural mismatch."""
    left: dict[str, list[Any]] = {"Category": ["A", "B"], "Total | n": [1, 2]}
    right: dict[str, list[Any]] = {"Total | n": [1, 2], "Category": ["A", "B"]}
...
def test_compare_tables_detects_structure_difference_for_row_count() -> None:
...
def test_compare_tables_applies_numeric_tolerance() -> None:
    """Treat numeric deviations within tolerance as identical."""
    left: dict[str, list[Any]] = {"id": [1, 2], "value": [10.0, 20.0]}
    right: dict[str, list[Any]] = {"id": [1, 2], "value": [10.005, 19.995]}

    within_tol = compare_tables(left, right, abs_tol=0.01)
    outside_tol = compare_tables(left, right, abs_tol=0.001)

    assert within_tol.category == "identical"
    assert outside_tol.category == "structure_differences"


def test_compare_tables_supports_ignore_case_and_spaces_flags() -> None:
    """Normalize case and spaces when flags are enabled."""
    left: dict[str, list[Any]] = {"id": ["01"], "term": ["Grade 3"]}
    right: dict[str, list[Any]] = {"id": ["01"], "term": ["grade    3"]}

    strict = compare_tables(left, right)
    normalized = compare_tables(
        left,
        right,
        ignore_case=True,
        ignore_spaces=True,
...
def test_compare_tables_handles_loaded_grouped_row_headers(test_data_dir: Path) -> None:
    """Compare grouped row-header tables without false differences."""
    table = load_rtf_table(
...
def test_compare_tables_stops_at_first_column_mismatch() -> None:
    """Return first mismatch based on column order and row position."""
    left: dict[str, list[Any]] = {"First": [1, 2], "Second": [10, 20]}
    right: dict[str, list[Any]] = {"First": [9, 2], "Second": [99, 20]}
